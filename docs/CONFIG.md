...
Two of those files are size-capped by the daemon itself (nothing else rotates them, since the log is written directly rather than through journald): the log is rewritten in place to its newest ~5 MB whenever it passes 10 MB, and the holdings history `<data_dir>/history.jsonl` rotates to `history.jsonl.1` when it passes 5 MB. The history records every addition (fill or swap, with the seed-scarcity roll that admitted the torrent), every swap's displaced set, and every drop (stalled zero-seeder eviction, vanished-from-catalog removal); `keep-at history` renders it with the same follow/`--all`/`--lines` semantics as `logs`.
...
Disk utilization is measured against keep-at's **configured storage limits** (the `storage`/`--storage-limit` totals), not raw filesystem usage - 100% means keep-at has reached the limit it was given. Disk *used* is **allocated** on-disk bytes under each location (what the host's quota actually charges): keep-at writes sparse files (an in-progress torrent's file is full length with holes where pieces haven't landed), so used lags committed while downloads and integrity checks are running and converges on it as data materializes. **Committed** is the storage the node has reserved for its held torrents - the sum of their nominal sizes - shown whenever it is nonzero. The gap between them is exactly the data still to materialize: `committed (100.0%)` with low `used` means the node is full *by reservation* and will fill as checks and downloads complete. Committed is what the scanner's free-space decisions budget against (plus a small fixed per-torrent buffer). "Since boot" means since this keep-at process started.
