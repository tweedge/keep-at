...
### Listener handshake cap raised to unbounded (upstream select! panic)

rqbit's listener task `select!`s between accepting a connection (branch disabled once pending handshake checks reach a cap, upstream default 256) and handshake completion, whose result is pattern-matched as `Some(Ok(..))` - a failed check (torrent still integrity-checking past the 5s live-wait, routine on large-library boots where every incoming peer hits that window) fails the pattern, silently disabling the branch. If the pending queue is at cap at that same instant, every select! branch is disabled and tokio panics - and with keep-at's `panic = "abort"` profile that SIGABRTs the whole daemon; the watchdog then restarts into the same window, so a busy swarm during a big boot can crash-loop. Observed live on a 4TB host (death-mark `PANIC at librqbit session.rs task_listener`), still unfixed upstream as of librqbit 9.0.1 and rqbit master (2026-09). keep-at raises `max_pending_incoming_handshake_checks` to `usize::MAX` (`listener_options` in `src/engine/session.rs`), which keeps the accept branch permanently enabled so the select! can never go all-disabled; the handshake queue is bounded in practice by inbound connection rate and each entry lives at most the 5s handshake-check timeout. If upstream ever fixes the select!, the cap can be restored to a sane bound.
...
- **Scrape results are cached across scans.** Each torrent's seeder/leecher counts persist in `scrape-cache.json` (TTL = the scan interval). A two-week re-scan reuses the previous cycle's counts instead of re-querying Academic Torrents' tracker for every catalog item, so repeat scans cost almost nothing.
...
keep-at therefore tracks download progress per held torrent: verified bytes on disk, and when that count last grew (persisted in state as `completed_pieces` and `last_progress_at`). Every scan refreshes both - from rqbit's `progress_bytes` when the torrent is managed, else from the on-disk dir size as a conservative proxy. A torrent that still has zero seeders **and** hasn't gained a single new byte since its stall clock started - for longer than `scan.stall_eviction_timeout` (default three months, configurable, `0` disables) - is removed to free its slot. The clock starts at first observation and resets whenever progress grows, so a torrent gets a full quiet window before any eviction, and a slow-but-alive download is never misclassified as stalled. Torrents still listed in the Academic Torrents catalog are the only candidates; one removed from the catalog is handled by the deleted-torrent path instead (below).

### Vanished-from-catalog torrents get the same quiet window

The deleted-torrent pass has its own destructive failure shape: a torrent missing from one catalog fetch is indistinguishable from one AT removed for good - and removal deletes downloaded data. Two defenses layer here. The coarse one is the collapse guard (mass disappearance refuses the whole pass). The fine one is `scan.vanished_eviction_timeout` (default three months, `0` restores next-scan removal, `preserve_deleted_torrents` disables the pass entirely): every scan stamps which held torrents the fresh catalog still lists (`last_confirmed_in_catalog_at` in state), and a torrent is removed only once it has stayed unlisted for the full window. The reasoning mirrors the stall timeout: a genuine removal is permanent, so waiting a quarter changes nothing about the eventual cleanup, while any transient - a partial fetch, AT-side schema work, an outage - lists the torrent again well within the window and resets the clock at zero cost. Stale state written before the field existed is priced from `added_at`, which for any real hold is far past the window, so upgrades keep the old next-scan behavior for pre-existing entries without a migration.
