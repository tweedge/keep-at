...
**Oversized torrents are disqualified before any work is done on them.** A torrent bigger than every storage location's *total* capacity can never be stored on the host, no matter how well it compresses or what keep-at displaces - so fetching its metadata, scraping its trackers, and probing its swarm are pure waste (the giant noaa-ncei datasets, up to 17TB against a 10GB cap, were exactly that). `ScanOnce` computes `maxFittableSize` (the largest single location's capacity) before the scrape begins, counts how many candidates exceed it, logs that disqualification at scrape start, and `evaluateCandidates` skips them entirely - they're never fetched, scraped, or probed.

**Scan memory is returned to the OS, not left at its peak.** A full-catalog scan parses gigabytes of `.torrent` metadata and the probe client accumulates per-piece bookkeeping for every probed candidate. Go's GC reclaims that heap, but the runtime returns freed memory to the OS lazily, so RSS can sit at the scan's peak long after the work finished - on a RAM-limited host that looks like a leak (observed: 7GB resident on a host whose held torrents account for ~1-2GB). `ScanOnce` now calls `debug.FreeOSMemory()` after the mid-scan probe-client resets and again at scan completion, forcing the reclaimed memory back to the OS so resident usage tracks what keep-at is actually holding rather than the scan's high-water mark.

**The swarm probe only runs for candidates keep-at actually accepts.** The probe (which adds the probed torrent to a dedicated probe client whose per-piece bookkeeping accumulates until the client is reset) used to run for *every* candidate with any seeders, before the seed-scarcity roll rejected most of them - so on a large catalog the probe client accumulated hundreds of torrents it never needed, the dominant RAM cost of a scan (measured with the debug collector: RSS climbed ~400MB over a partial scan while held torrents grew by 44, and goroutines multiplied per probed torrent). The roll now runs first and the probe happens only for candidates that are actually added or swapped (see `actOnCandidate`/`probeAcceptedCandidate`), and the mid-scan probe-client reset is keyed on the number of probes since the last reset rather than candidates processed. The keep-at peer count was already documented as network-status metadata that doesn't gate selection, so this only changes *which* swarms get counted, not *what* keep-at chooses - and it means network-status counts peers on torrents keep-at actually seeds, which is the meaningful subset.
