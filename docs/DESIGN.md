...
Counting other keep-at nodes is a separate, explicit, on-demand operation: `keep-at network-status` (see `engine.RunCensus`). It walks the whole catalog and, per torrent, fetches the metadata, scrapes AT's tracker, and briefly joins the swarm to record, per keep-at peer found: its (best-effort) node identity, and whether it has the whole torrent (seeding) or not (leeching). The command prints a running status line every 25 torrents and a summary when complete. This is RAM- and time-heavy by design - it joins every torrent's swarm, and each probe can wait up to the probe timeout (10s default) for peers - so it's something an operator runs when they want the network picture, never something the node does as part of normal operation.

The census also computes the **p10 seeder floor** (x from the seed-scarcity gate, see "Seeding minimally-seeded torrents" above) fresh from its own scrapes, the same figure scans persist for the next scan's anchor.
...
Because the census is synchronous and probes one torrent at a time with a fresh probe client per torrent, its probe-client memory never accumulates beyond a single torrent's piece bookkeeping - the 250-torrent batching that bounded the old in-scan probe memory is gone entirely, along with the mid-scan reset machinery (see "Why probing uses a second, disposable torrent client" below).
...
### Why probing uses a second, disposable torrent client (only in the census)

Independent of the above, probing a torrent's swarm (see "Network-wide stats") works by adding a torrent just to inspect its peers - and earlier versions of this code dropped it again immediately afterward. Rapid add-then-drop across thousands of candidates per scan is exactly the kind of churn that triggers bugs like the one above, library version notwithstanding.

Probing no longer happens during scans at all - the node trusts the tracker. It only happens in the explicit `keep-at network-status` census, which uses a dedicated `*torrent.Client` per probed torrent that:
...
* Is created fresh for every torrent and closed as soon as that torrent's probe completes (or times out). Because probes are strictly synchronous, nothing is ever in flight against the client when it's closed - so the add/drop race the old code dodged by accumulating torrents can't occur here at all, and peak probe-client memory is one torrent's bookkeeping rather than hundreds. The old mid-scan reset machinery (`resetProbeClient` every 250 candidates) is gone with the in-scan probing it bounded.

DHT stays enabled on the main client: disabling it globally was tried first and measurably hurt real download connectivity (one torrent went from a 15-second download to not finishing within 90 seconds with DHT off). Isolating the census's churn to a disposable, DHT-free client keeps the crash risk contained without that cost.

### The census is RAM-heavy by design, and bounded

`keep-at network-status` joins every torrent's swarm - that's what makes it slow (each probe waits up to the probe timeout for peers) and RAM-heavy (a torrent's piece-level bookkeeping - hashes and per-piece state, sized by piece *count* - can run into tens of megabytes for large datasets). It's an operator-invoked, on-demand command for that reason, not something the node does. Its memory is nonetheless bounded: the per-torrent probe-client lifecycle above means it never holds more than one probed torrent's bookkeeping at a time.

### Evaluating candidates concurrently

`evaluateCandidates` evaluates up to `evaluateConcurrency` (16) candidates at once; the actual requests to Academic Torrents (fetching `.torrent` files, scraping trackers) stay correctly rate-limited regardless, since they share one `rate.Limiter` across every goroutine.
...
- **Scrape results are cached across scans.** `scrapeSwarm` remembers each torrent's seeder/leecher counts in `scrape-cache.json` (TTL = the scan interval). A weekly re-scan reuses last week's counts instead of re-querying Academic Torrents' tracker for every catalog item, so repeat scans cost almost nothing.
...
**The catalog is walked in shuffled order.** Academic Torrents' `database.xml` groups torrents by upload/series, so giant datasets cluster into contiguous runs - hundreds of datasets over 100 GB, e.g. the whole noaa-ncei block. Walking it in that order made `evaluateCandidates`' 16 concurrent workers all fetch and parse multi-megabyte `.torrent` files (tens of thousands of piece hashes each) at once, spiking CPU and RAM together and stalling the scrape on exactly those segments (observed on a live node: 228 of 2816 candidates in 47 minutes, stuck at the noaa-ncei cluster). `ScanOnce` now shuffles the catalog's items before evaluation, spreading the giants across the whole scan so only a few are in flight at any moment. It's a pure permutation - every candidate is still evaluated and ranked exactly once - and the incremental acting described above still fills the top slots early, just no longer biased toward whatever cluster happened to come first in the XML.

**Oversized torrents are disqualified before any work is done on them.** A torrent bigger than every storage location's *total* capacity can never be stored on the host, no matter how well it compresses or what keep-at displaces - so fetching its metadata, scraping its trackers, and evaluating it are pure waste (the giant noaa-ncei datasets, up to 17TB against a 10GB cap, were exactly that). `ScanOnce` computes `maxFittableSize` (the largest single location's capacity) before the scrape begins, counts how many candidates exceed it, logs that disqualification at scrape start, and `evaluateCandidates` skips them entirely - they're never fetched or scraped.

**Scan memory is returned to the OS, not left at its peak.** A full-catalog scan parses gigabytes of `.torrent` metadata over its lifetime. Go's GC reclaims that heap, but the runtime returns freed memory to the OS lazily, so RSS can sit at the scan's peak long after the work finished - on a RAM-limited host that looks like a leak (observed: 7GB resident on a host whose held torrents account for ~1-2GB). `ScanOnce` calls `debug.FreeOSMemory()` at scan completion, forcing the reclaimed memory back to the OS so resident usage tracks what keep-at is actually holding rather than the scan's high-water mark.

**The Go runtime is told the RAM budget.** keep-at computes a RAM budget (80% of system RAM by default, `--max-ram` to override) and caps the torrent count from it, but never told the Go runtime about it - so the GC kept ~2x the live heap (GOGC default) and held freed pages resident, letting RSS wander well past the budget between the explicit `FreeOSMemory` calls. `Engine.New` now calls `debug.SetMemoryLimit(budget)` so the GC is actively bounded by the same number keep-at plans around, keeping RSS within roughly the budget plus overhead even during the churn of a long scan.

**Diagnostics are built in.** `debug: true` (config) / `--debug` (CLI) turns on debug-level logging and a collection loop that writes under `<data_dir>/debug/`: a `memory.jsonl` line every minute (process RSS, Go heap, goroutines, GC pauses, held/seeding counts), plus a heap pprof, full goroutine dump, and short CPU trace every 15 minutes (rotated to keep the newest 20 of each). The heap profile is the definitive "what is holding this RAM" answer and the memory curve the definitive "when" - a directory a remote operator can leave running for the hours it takes the failure to recur and hand over for analysis. The regular `runtime stats` log line and `keep-at status` also now report RSS, Go heap, and goroutine count without needing debug mode.

### Why a seeding node's CPU was pegged - and what fixed it

A host holding 827 torrents was reporting CPU pegged at 100% while transferring only a handful of MB per 30 minutes. The debug collector's CPU profiles (added alongside this investigation) answered it: with ~400 held torrents, roughly **22% of CPU was the library's tracker-announce dispatcher, ~20% the uTP packet reader, and ~16% peer-connection setup** - all machinery, none of it data transfer. Three changes removed it:

- **Held torrents only announce to Academic Torrents' own tracker** (see `atTrackersOnly` in `addTorrentSpec`). The anacrolix client re-announces to every tracker in a torrent's spec on its own schedule, and AT catalog entries list up to a dozen mostly-dead third-party trackers each - so every held torrent was cycling announce timeouts against dead public trackers all day. keep-at is an AT seeder; AT's tracker and DHT are its peer discovery, and the scrape path was already AT-first. This alone removed the dispatcher from the profile's top.
- **uTP is disabled** (`DisableUTP`). The UDP-based transport's packet-reader goroutine was ~20% of CPU for a seeder whose peers are overwhelmingly plain TCP.
- **Peer pools are smaller** (`TorrentPeersHighWater=100`, `TorrentPeersLowWater=20` vs the library's 500/50). Under-seeded torrents - keep-at's entire purpose - have thin peer pools, and with the library defaults every one of them kept re-announcing and dialing addresses that don't exist. Smaller pools bound that churn without hurting real connectivity: a 100-address pool is still far larger than any real swarm keep-at joins.

Measured on a 254-torrent node after these changes: **5-8% of one core while idle-seeding**, down from **107-124%** (the old build's figure was also at ~400 held, so the comparison is apples-to-apples on the per-torrent cost). Goroutines dropped from ~14,800 to ~7,900. The connection-buffer values (`establishedConnsPerTorrent=12` etc.) were investigated as the suspected cause and are **not** the problem - they're the RAM lever, and raising them toward the library's defaults (50 connections × 1MiB per torrent) would multiply per-torrent memory, which is the opposite of what a RAM-limited host needs. The churn was dial frequency against nonexistent peers, not buffer size.

### Verified end to end

After the fixes above, a real run against the entire live catalog (2,850 items, 10GB cap in `/tmp`) completed its first full scan in 4h19m with zero crashes and bounded memory, selected 538 torrents to hold, and stayed stable for another 2+ hours of real downloading and seeding afterward. Every fix above was found and confirmed this way - none of them reproduced in anything smaller than the real catalog at real duration.
...
- **`KEEPAT_SMOKE_TEST=1`** - the fast one. Two hand-picked, verified-seeded torrents through the whole pipeline (catalog -> `.torrent` fetch -> scrape -> download -> store) in under a minute. Good for a quick "is the plumbing connected" check, but too small to exercise anything scale-dependent.
