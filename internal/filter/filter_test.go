...
func TestAgeEligible(t *testing.T) {
	...
	cases := []struct {
		...
		{"exactly at boundary", now.Add(-minAge), true},
		{"older than boundary", now.Add(-minAge - time.Hour), true},
		{"younger than boundary", now.Add(-minAge + time.Hour), false},
		{"unknown age", time.Time{}, false},
	}

	for _, c := range cases {
		got := AgeEligible(c.createdAt, minAge, now)
		if got != c.want {
