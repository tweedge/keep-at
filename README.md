...
`history` prints what keep-at has done to your holdings: additions (fill or swap, with the seed-scarcity roll that admitted each torrent), swaps (what was displaced and by what), and drops (stalled zero-seeder torrents, torrents that vanished from the Academic Torrents catalog). Same follow-by-default semantics as `logs` with `--all` and `--lines N`, plus `--no-follow` to print the last N events and exit; additions render green, swaps yellow, drops red when stdout is a terminal. It reads `<data_dir>/history.jsonl` (rotated at 5 MB, one previous generation kept) and works whether the daemon is running or not.
...
Recommended provisioning is **512 MiB of RAM as a base, plus ~128 MiB per TB of storage** - about 1 GiB of RAM for a 4 TB node, or 2 GB of RAM for a 12 TB node, and so on. A node provisioned at that line sits at the large-lean knee of the bias (bias ≈ +1): it holds fewer-but-larger torrents, spending each scarce RAM slot on as many bytes as possible, and that large-lean selection is what lets the budget stretch this far. More RAM than the line moves the bias negative and the node holds numerous-but-smaller torrents with higher RAM cost each. Either way the RAM budget is a hard ceiling - free-space fill still refuses any candidate whose RAM price exceeds remaining headroom, and swaps still require the displaced set to cover the candidate's RAM cost - so the node can never spend its way into an OOM.
...
Recommended provisioning: **512 MiB of physical RAM as a base, plus ~128 MiB per TB of storage** - about 1 GiB for a 4 TB node. Measured production nodes run far inside that line: with DHT permanently disabled, a 3.8 TiB library holding the catalog's most piece-dense selection sits at ~400 MiB RSS, and a smaller node's 3.5 TiB library at ~115 MiB, so the recommendation is a safety floor, not a typical-use figure. RAM cost tracks torrent count and piece count, while disk tracks bytes, so what the slots total in bytes depends on *which* torrents urgency ranking selects: the same ~1,190 slots hold ~20 GB at a 16 MB torrent average, or ~2 TB at the 1.72 GB average of the catalog's 600 largest entries. The size bias steers RAM-short hosts toward the latter, which is what lets 1 GiB cover ~4 TB.

Two notes on the shape of that line. First, the per-TB figure assumes a typical mix of piece sizes: real RAM cost is per piece (~64 B each), and piece density varies widely across the catalog, so an unusually piece-dense selection can price far above 128 MiB/TB. That case is caught by keep-at's per-candidate RAM pricing rather than by this rule of thumb - the disk sits partly empty by design rather than the node OOMing. Second, RAM above the minimum is not wasted: the OS puts it to work as page cache over the seeded data, which matters for seek-heavy hard drives.

Less RAM than the recommendation works by finding and prioritizing fewer-but-larger torrents with lower RAM cost per byte (bias toward +1): a 512 MiB + 1 TB box holds ~600 large torrents and seeds hundreds of GB usefully within budget - under the provisioning line, it leans on the RAM budget and a partly-filled disk. More RAM works by finding and prioritizing numerous-but-smaller torrents with higher RAM cost each (bias toward −1): a 64 GiB box spends its ~50k slots on the small end of the catalog the big hosts skip. On a RAM-short box the disk will sit partly empty by design - free-space fill refuses anything whose RAM price exceeds remaining headroom, because the alternative is exceeding the RAM budget and OOMing the host. If the disk must be full, add RAM, not flags: no selection parameter can hold more torrents than the budget prices.
